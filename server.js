...
const { getAssociatedTokenAddressSync, createAssociatedTokenAccountInstruction, createSyncNativeInstruction, createCloseAccountInstruction } = require("@solana/spl-token");

dotenv.config();

const VERSION = "1.5.9-fix17-send-setup-swap";
const app = express();
...
app.post("/build-swap-tx", async (req, res) => {
  try {
    ...
    if (closeWsol) tx.add(createCloseAccountInstruction(wsolAta, user, user));

    const b64 = await finalizeTx(tx, user, true);
    res.json({ ok:true, version:VERSION, pool:built.poolId.toBase58(), poolType:built.poolType, usdcAta:built.usdcAta.toBase58(), wsolAta:built.wsolAta.toBase58(), tx:b64, warning:"Swaps are not guaranteed profitable. Use minAmountOutUsdc for protection." });
  } catch (e) {
    res.status(400).json({ ok:false, error:e.message, debug:{ rawBody:req.rawBody||"", jsonError:req.jsonError||null, parsedBody:req.jsonBody||null } });
  }
});

...
const VERSION = "1.5.6-fix14-setup-swap-textjson";
const app = express();
...
/**
 * Robust body parsing: accept any content-type as text, then parse JSON ourselves.
 * This avoids express.json strict/proxy quirks and prevents "Invalid arguments" surprises.
 */
  ...
  if (req.method === "POST" || req.method === "PUT" || req.method === "PATCH") {
    ...
    if (raw && raw.trim().length) {
      try {
        ...
        req.jsonBody = null;
        req.jsonError = String(e && e.message ? e.message : e);
      }
    } else {
      req.jsonBody = null;
    }
...
async function finalizeTx(tx, feePayer, doSimulate=true) {
...
async function decodeClmmPoolState(sdk, poolId) {
  ...
  return { decoded, decodedKeys: Object.keys(decoded), pubkeys: extractPubkeys(decoded) };
}
...
async function buildClmmSwapIxs({ owner, amountInLamports, minAmountOutUsdc }) {
  ...
  // require WSOL ATA exists (setup first)
  const wsolInfo = await connection.getAccountInfo(wsolAta);
  if (!wsolInfo) throw new Error("WSOL ATA does not exist yet. Call POST /build-setup-tx first.");
...
// Routes
app.get("/", (_req, res) => res.json({ ok: true, version: VERSION, endpoints: ["/version","/debug/echo","/try-fetch","/derive/:wallet","/build-setup-tx","/build-swap-tx"] }));
...
app.post("/debug/echo", (req, res) => {
  res.json({
    ...
    rawBody: req.rawBody || "",
    jsonError: req.jsonError || null,
    parsedBody: req.jsonBody || null,
    headers: req.headers,
  });
});
...
// Setup: create ATAs + wrap SOL into WSOL ATA
...
app.post("/build-swap-tx", async (req, res) => {
  try {
    ...
    const b64 = await finalizeTx(tx, user, true);
    res.json({ ok: true, version: VERSION, pool: built.poolId.toBase58(), poolType: built.poolType, usdcAta: usdcAta.toBase58(), wsolAta: wsolAta.toBase58(), tx: b64 });
  } catch (e) {
    res.status(400).json({ ok: false, error: e.message, debug: { rawBody: req.rawBody || "", jsonError: req.jsonError || null, parsedBody: req.jsonBody || null } });
  }
});

...
const VERSION = "1.5.7-fix15-nosim-setup";
const app = express();
...
// Minimal endpoints
app.get("/", (_req, res) => res.json({ ok: true, version: VERSION, endpoints: ["/version","/debug/echo","/derive/:wallet","/build-setup-tx"] }));
app.get("/version", (_req, res) => res.json({ ok: true, version: VERSION }));
app.post("/debug/echo", (req, res) => res.json({ ok: true, version: VERSION, rawBody: req.rawBody || "", jsonError: req.jsonError || null, parsedBody: req.jsonBody || null, headers: req.headers }));
...
// Setup tx WITHOUT simulation (avoids RPC simulate "Invalid arguments" weirdness)
app.post("/build-setup-tx", async (req, res) => {
  try {
    ...
    res.json({ ok: true, version: VERSION, usdcAta: usdcAta.toBase58(), wsolAta: wsolAta.toBase58(), wrapLamports: Math.trunc(wrapLamports), tx: b64, note: "No simulation performed on setup tx. Sign+send in Phantom; if it fails, we’ll read the on-chain error." });
  } catch (e) {
